...
async function run() {
  ...
  // For monorepos, run install once at root; otherwise run in each modified directory
  if (isMonorepo(cwd)) {
    ...
    for (const dir of modifiedDirs) {
      ...
      console.log(c('dim', `📦 ${relativeDir} (${packageManager})`));
      
      const installSuccess = runInstall(packageManager, dir);
      if (!installSuccess) {
        allInstallsSucceeded = false;
      }
    }
