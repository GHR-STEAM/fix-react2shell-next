...
function findProjectRoot(startDir, stopAt) {
  let dir = startDir;
  
  while (dir.length >= stopAt.length && dir !== path.dirname(dir)) {
    if (hasLockFile(dir)) {
      return dir;
    }
...
async function run() {
  ...
  // For monorepos, run install once at root
  if (isMonorepo(cwd)) {
    const packageManager = detectPackageManager(cwd);
    console.log(c('dim', `📦 Monorepo detected (${packageManager})`));
    ...
    for (const dir of modifiedDirs) {
      const root = findProjectRoot(dir, cwd);
