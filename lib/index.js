...
function getInstalledVersion(pkgDir, packageName) {
  // Try package manager first (handles catalogs, overrides, etc.)
...
function analyzePackageJson(pkgPath) {
  ...
  if (allDeps.next) {
    ...
    // If version spec is unparseable (catalog:, latest, etc.), resolve via package manager
    if (isCatalogReference(version) || isUnparseableVersionSpec(version) || !parseVersion(version)) {
      resolvedVersion = getInstalledVersion(pkgDir, 'next');
      if (resolvedVersion) {
    ...
    if (check.vulnerable) {
      ...
      vulnerabilities.push({
        ...
        patched: '15.5.7',
        note: 'Could not resolve version - run your package manager install first, or pin to a safe version',
        inDeps: !!pkg.dependencies?.next,
        inDevDeps: !!pkg.devDependencies?.next,
      });
    }
...
function applyFixes(pkgPath, vulnerabilities) {
  ...
  for (const vuln of vulnerabilities) {
    ...
    // Pin exact version - using ^ can resolve to a different vulnerable minor version
...
async function run() {
  ...
  // Check if we're inside a monorepo (walk up from cwd)
  ...
  if (monorepoRoot) {
    ...
    const installSuccess = runInstall(packageManager, monorepoRoot);
    if (!installSuccess) {
      allInstallsSucceeded = false;
    }
  } else {
    // Find project roots (where lock files are) for each modified directory
    ...
    for (const root of projectRoots) {
      ...
      const installSuccess = runInstall(packageManager, root);
      if (!installSuccess) {
        allInstallsSucceeded = false;
      }
    }
  ...
  if (allInstallsSucceeded) {
    ...
    console.log(c('yellow', '\n⚠️  Some install commands had issues.'));
