...
function getPnpmCatalogVersion(pkgDir, packageName, catalogSpec) {
  const workspaceYamlPath = findPnpmWorkspaceYaml(pkgDir);
  if (!workspaceYamlPath) return null;
  
  try {
    const content = fs.readFileSync(workspaceYamlPath, 'utf8');
    const workspace = parseSimpleYaml(content);
    
    const catalogName = catalogSpec.slice(8).trim(); // Remove "catalog:" prefix
    
    if (!catalogName || catalogName === 'default') {
      if (workspace.catalog && workspace.catalog[packageName]) {
        return workspace.catalog[packageName];
      }
      if (workspace.catalogs?.default?.[packageName]) {
        return workspace.catalogs.default[packageName];
      }
    } else {
      if (workspace.catalogs?.[catalogName]?.[packageName]) {
...
function isCatalogReference(version) {
  return version && version.startsWith('catalog:');
}
...
function analyzePackageJson(pkgPath) {
  ...
  if (allDeps.next) {
    ...
    // Handle pnpm catalog references
    if (isCatalogReference(version)) {
      const catalogVersion = getPnpmCatalogVersion(pkgDir, 'next', version);
      if (catalogVersion) {
        displayVersion = `${version} (catalog: ${catalogVersion})`;
        version = catalogVersion;
      } else {
        // Fall back to installed version if catalog lookup fails
    ...
    } else if (isUnparseableVersionSpec(version) || !parseVersion(version)) {
      installedVersion = getInstalledVersion(pkgDir, 'next');
      if (installedVersion) {
        displayVersion = `${version} (installed: ${installedVersion})`;
        version = installedVersion;
    ...
    } else if (isCatalogReference(allDeps.next) && !getPnpmCatalogVersion(pkgDir, 'next', allDeps.next) && !installedVersion) {
      // Catalog reference that couldn't be resolved
      vulnerabilities.push({
        package: 'next',
        current: allDeps.next,
        patched: '15.5.7',
        note: 'Could not resolve pnpm catalog version - check pnpm-workspace.yaml or run "pnpm install" first',
        inDeps: !!pkg.dependencies?.next,
        inDevDeps: !!pkg.devDependencies?.next,
      });
  ...
  for (const rscPkg of REACT_RSC_PACKAGES) {
    if (allDeps[rscPkg]) {
      ...
      // Handle pnpm catalog references
      if (isCatalogReference(version)) {
        const catalogVersion = getPnpmCatalogVersion(pkgDir, rscPkg, version);
        if (catalogVersion) {
          displayVersion = `${version} (catalog: ${catalogVersion})`;
          version = catalogVersion;
        } else {
      ...
      } else if (isUnparseableVersionSpec(version)) {
        const installedVersion = getInstalledVersion(pkgDir, rscPkg);
        if (installedVersion) {
          displayVersion = `${version} (installed: ${installedVersion})`;
          version = installedVersion;
      ...
      } else if (isCatalogReference(allDeps[rscPkg]) && !getPnpmCatalogVersion(pkgDir, rscPkg, allDeps[rscPkg]) && !getInstalledVersion(pkgDir, rscPkg)) {
        // Catalog reference that couldn't be resolved
        vulnerabilities.push({
          package: rscPkg,
...
module.exports = { run, findAllPackageJsons, analyzePackageJson, isNextVersionVulnerable, getPnpmCatalogVersion, isCatalogReference };
